...
export const data_load = (bool)=>{
    return async (dispatch)=>{
        ...
        if(rdata.status){
            ...
            dispatch({ type: "VIRTUAL_GET_ALL_DATA", data: gamelist});            
            var pros = [];
            var types = [{label : "ALL",value : "All"}];
            if(pros){
                for(var i = 0 ; i < pdata.length ; i++){
                    pros.push({label : pdata[i].provider.toLocaleUpperCase(),value : pdata[i].provider}, dispatch);
                }
                dispatch({
...
export const providerchange = (value,bool)=>{
    return async(dispatch,getState) =>{
        ...
        if(returndata.status){
            dispatch({
                type : "VIRTUAL_SETPROVIDER",setprovider : value
            });
...
export const gametypechange = (value)=>{
    return async(dispatch,getState)=>{        
        dispatch({
            type : "VIRTUAL_TYPE",data : {label : value ,value:value}
        });
        var allData  = getState().virtuallist.allData;
        var filteredData = Filter(value,allData)        
        dispatch({
            type : "VIRTUAL_GET_DATA",
            data : filteredData
        })
    }
}
...
export const filterData = value => {
  return dispatch => dispatch({ type: "VIRTUAL_FILTER_DATA", value })
}
